...
file_db <- readRDS("inst/couple-Atlantis/coupling_versionJanu2019/run.3/file_db.rds")
## GRRR 
...
doitfun <- function(itime) {
  ...
  delif(salt0)
  delif(vert0)
box_summ <- romstab %>% inner_join(box_index[c("cell_", "boxid")], c("cellindex" = "cell_")) %>% 
...
face_summ <- romstab %>% dplyr::select(-temp, -salt, -depth) %>% 
